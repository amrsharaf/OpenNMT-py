...
parser = argparse.ArgumentParser(description='run_pytorch.py')
...
def get_accuracy(output, target):
    ...
    predictions = (output >= 0.5).type_as(target)
...
#     correct_output = thresholded_output == target
#     return len(correct_output) / len(target)
    ...
    def trainEpoch(epoch):
        ...
        for i in range(len(trainData)):
            ...
            if opt.adapt:
                ...
                accuracy = get_accuracy(torch.cat([old_domain, new_domain]).data, discriminator_targets.data)
            ...
            if i % opt.log_interval == 0 and i > 0:
                print("Epoch %2d, %5d/%5d batches; %3.0f Source tokens/s; %6.0f s elapsed" %
